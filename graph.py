...
class GetData:
    ...
    def extract_json_data(plot_data, kpi):
        ...
        for i in range(len(algorithms)):  # 0 - 7
            ...
            for j in range(len(datasets)):  # A - T
                p_d = plot_data[algorithms[i]][datasets[j]][kpi]
                if isOnline:
                    online_data[j].append(float(p_d))
                elif not isOnline:
                    offline_data[j].append(float(p_d))
    ...
    def average_data(plot_data):
        ...
        average_online_data = [0 for _ in range(len(online_data[0]))]
        average_offline_data = [0 for _ in range(len(offline_data[0]))]
        ...
        avg_plot_data = (average_online_data,
                         average_offline_data,
                         algorithms, datasets)
